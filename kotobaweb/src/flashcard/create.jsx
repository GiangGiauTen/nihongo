...
import socketEvents from '../common/flash_card/socket_events';
...
class Create extends PureComponent {
  ...
  componentDidMount() {
    this.socket = createSocket(SocketNamespaces.Flash_Card);
    Analytics.setPageView('/flashcard/create');
  }
  ...
  submitCreate = (values) => {
    ...
    this.socket.on(socketEvents.Server.CREATED_GAME, (response) => {
      history.push(`/flashcard/game?username=${encodeURIComponent(values.username)}&gameID=${response}`);
    });
  ...
  render() {
    return (
      <div>
        ...
        <div className="container-fluid mt-4" id="createFlashCardContainer">
          <TabBar tabs={tabs} />
          <Formik
            initialValues={{
              answerTimeLimit: 30,
